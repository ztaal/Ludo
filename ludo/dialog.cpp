    ...
    ui(new Ui::Dialog)
{
    ...
    std::vector<std::pair<QColor,QColor> >base_colors {
        std::make_pair(QColor(0, 0, 0),QColor(0, 0, 0)),  //G
    ...
    // Insert globe at goal
    QGraphicsPixmapItem * tmp = new QGraphicsPixmapItem( QPixmap::fromImage(globe_img));
    tmp->setPos(415, 245);
    tmp->setScale(1.6);
    scene->addItem(tmp);
...
void Dialog::create_graphic_players(){
    ...
    for(int c = 0; c<4; ++c){
        if(c == 0){
            ...
            piece = QBrush(QColor(Qt::darkMagenta));
        } else if(c == 1){
